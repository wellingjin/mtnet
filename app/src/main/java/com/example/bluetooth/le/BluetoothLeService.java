	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onCharacteristicChanged(BluetoothGatt gatt,
			...
			if (data != null && data.length > 0){												
				...
				Log.i("asdfg", "测量数据为："+ stringBuilder.toString());
				while(count>=6){
					if(bufferint.get(0)==254){
						if(bufferint.get(1)==132){
							...
							case 1:update_date(ERROR_1);Log.i("asdfg_info", "测量过程中没有侦测到脉搏信号");break;
							case 2:update_date(ERROR_2);Log.i("asdfg_info", "测量错误，测量过长中干扰过大");break;
							case 3:update_date(ERROR_3);Log.i("asdfg_info", "充气失败，充气时间过长或袖带漏气");break;
							case 5:update_date(ERROR_5);Log.i("asdfg_info", "测量失败，测量的结果高压与低压相差太大");break;
							default:update_date(ERROR_4);Log.i("asdfg_info", "测量过程中出现未知错误");break;
							}
							for(int i=0;i<6;i++)bufferint.remove(0);
							count-=6;
						}
							...
							Log.i("asdfg_info", "电量过低，请更换电池或插上电源");
							for(int i=0;i<6;i++)bufferint.remove(0);
							count-=6;
						}
					}
